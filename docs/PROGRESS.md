...
## v1.4.5 — 地点日期与 Day N（2026-09-10）

- 地点每次访问的日期作为实际安排，线路日期按地点推导。
- 首次为某个地点或线路定日期时，按照原 Day N 偏移展开整条线路；给已定日期线路中的单个地点设日期不会影响未安排项。
- 清空线路日期时清空全部地点起止日期并恢复 Day N，保留空缺天数和重复访问。
- 修复 Day N 保存后丢失；日期及日序号保存在每次地点引用下，适配共享地点和重复访问。
- 修复全无日期线路的分组、Day N 标签编号和首次拖拽设日期的联动。
- 验证：227 项测试、TypeScript 检查与生产构建通过。
