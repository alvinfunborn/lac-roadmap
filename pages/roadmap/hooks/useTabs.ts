...
import { t } from '../../../i18n';
import { dayNumber, isDatedRoadmap, scheduleGroupKey } from '../../../utils/schedule';
...
export function useTabs({ data, groups, groupKeys, tempDayKeys, subEndpoints }: UseTabsParams): TabsApi {
  ...
  const tabDefs = useMemo<TabDef[]>(() => {
    ...
    allKeysForTabs.forEach((k, i) => {
      ...
      const label = isDateKey(k) ? k : t('tabs.day.label', { n: Number(k.match(/第(\d+)天/)?.[1]) || i + 1 });
  ...
  const visibleItemIndices = useMemo(() => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const key = scheduleGroupKey(p, dated);
